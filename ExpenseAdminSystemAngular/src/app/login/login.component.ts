  ...
  login() {
    ...
    this.auth.authenticate(this.username, this.password).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        this.isLoading = false;
        // Create a new Error object from the response for consistent error handling
        const errorObj = new Error(err.error?.message || err.message);
        console.error('AuthService error:', err); // Keep the original HttpErrorResponse
        console.error('Authentication failed:', errorObj); // Add the expected Error object
